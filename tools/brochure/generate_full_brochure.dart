...
void _addTracingPages(BrochureContext ctx, Map<String, dynamic> lesson, void Function(String) addPage, int Function() getPageNum) {
  final tasks = (lesson['sections'] as List).expand((s) => s['tasks'] as List).toList();
  final tiles = tasks.map((t) {
    final content = t['content'] as Map<String, dynamic>;
    return BrochureEngine.tracingLetterSvg(content['letter'], content['checkpoints']);
  }).join();

  addPage(BrochureEngine.contentPage(
    title: 'Tracing',
...
void _addSpellingPages(BrochureContext ctx, Map<String, dynamic> lesson, void Function(String) addPage, int Function() getPageNum) {
  final tasks = (lesson['sections'] as List).expand((s) => s['tasks'] as List).toList();
  ...
  _paginate(uniqueItems.entries.toList(), 20, getPageNum, addPage, (chunk) {
    return chunk.map((e) {
      return '<div class="content-tile"><span class="tile-emoji">${e.value}</span><span class="tile-text">${e.key}</span></div>';
    }).join();
  }, 'Word Building', 'Master spelling with essential Punjabi words', 'VOCABULARY LIST');
}

void _addMatchingImagesPages(BrochureContext ctx, Map<String, dynamic> lesson, void Function(String) addPage, int Function() getPageNum) {
  final tasks = (lesson['sections'] as List).expand((s) => s['tasks'] as List).toList();
  final uniqueItems = <String, String>{};
  for (final t in tasks) {
    final c = t['content'] as Map<String, dynamic>;
  ...
  }, 'Visual Vocabulary', 'Connecting images to Gurmukhi script', 'PICTURE MATCHING');
}

void _addMatchingWordsPages(BrochureContext ctx, Map<String, dynamic> lesson, void Function(String) addPage, int Function() getPageNum) {
  final tasks = (lesson['sections'] as List).expand((s) => s['tasks'] as List).toList();
  ...
  _paginate(allPairs.entries.toList(), 24, getPageNum, addPage, (chunk) {
    return chunk.map((e) => '<div class="content-tile"><span class="tile-text">${e.key}</span><span class="tile-sub">${e.value}</span></div>').join();
  }, 'Word Recognition', 'Building reading confidence through repetition', 'READING PRACTICE');
}

void _addFillInBlankPages(BrochureContext ctx, Map<String, dynamic> lesson, void Function(String) addPage, int Function() getPageNum) {
  final tasks = (lesson['sections'] as List).expand((s) => s['tasks'] as List).toList();
  ...
  _paginate(uniqueWords.toList()..sort(), 24, getPageNum, addPage, (chunk) {
    return chunk.map((w) => '<div class="content-tile"><span class="tile-text">$w</span></div>').join();
  }, 'Contextual Learning', 'Completing words within real-world contexts', 'FILL IN THE BLANKS');
}

void _addSentencesPages(BrochureContext ctx, Map<String, dynamic> lesson, void Function(String) addPage, int Function() getPageNum) {
  final tasks = (lesson['sections'] as List).expand((s) => s['tasks'] as List).toList();
  _paginate(tasks, 10, getPageNum, addPage, (chunk) {
    return chunk.map((t) => '<div class="content-tile sentence-tile"><span class="tile-text">${t['content']['fullSentence']}</span></div>').join();
  }, 'Sentence Construction', 'Building complex thoughts from single words', 'CONVERSATIONAL PUNJABI', gridClass: 'sentences');
}

void _addCrosswordPages(BrochureContext ctx, Map<String, dynamic> game, void Function(String) addPage, int Function() getPageNum) {
  ...
  _paginate(allWords.entries.toList(), 24, getPageNum, addPage, (chunk) {
    return chunk.map((e) => '<div class="content-tile"><span class="tile-text">${e.key}</span><span class="tile-sub">${e.value}</span></div>').join();
  }, 'Crossword Challenges', 'Reinforcing vocabulary through logic puzzles', 'GAME VOCABULARY', dark: true);
}

void _addBubblePopWordsPages(BrochureContext ctx, Map<String, dynamic> game, PageAdder addPage, int Function() getPageNum) {
  final allWords = (game['content']['itemPool'] as Map).keys.toList();
  _paginate(allWords..sort(), 24, getPageNum, addPage, (chunk) {
    return chunk.map((w) => '<div class="content-tile"><span class="tile-text">$w</span></div>').join();
  }, 'Arcade: Bubble Pop', 'Dynamic word identification under pressure', 'GAME VOCABULARY', dark: true);
}
...
void _paginate<T>(
  ...
  for (var i = 0; i < items.length; i += perPage) {
    final chunk = items.sublist(i, (i + perPage) > items.length ? items.length : i + perPage);
    addPage(BrochureEngine.contentPage(
      title: title,
      subtitle: subtitle,
      bodyHtml: '<div class="content-grid $gridClass">${renderer(chunk)}</div>',
      footerLeft: footer,
      pageNum: getPageNum(),
      dark: dark,
    ));
